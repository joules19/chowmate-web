...
import LowFrequencyCustomerReportFilters from "./LowFrequencyCustomerReportFilters";
import CSVPreviewTable from "./CSVPreviewTable";
import {
  ReportRepository,
  LowFrequencyCustomerFilterDto,
} from "@/app/lib/api/repositories/report-repository";

export default function LowFrequencyCustomerReportCard() {
  const [isFiltersOpen, setIsFiltersOpen] = useState(false);
  const [isPreviewOpen, setIsPreviewOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [csvData, setCsvData] = useState<string>("");
  const [currentFilters, setCurrentFilters] = useState<LowFrequencyCustomerFilterDto>({});

  const reportRepository = new ReportRepository();

  const handleApplyFilters = async (filters: LowFrequencyCustomerFilterDto) => {
  ...
  const handlePreview = async () => {
    setIsLoading(true);
    try {
      const data = await reportRepository.getLowFrequencyCustomerReportPreview(currentFilters);
      setCsvData(data);
  ...
  const handleDownload = async () => {
    setIsLoading(true);
    try {
      const blob = await reportRepository.downloadLowFrequencyCustomerReportCsv(currentFilters);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `low_frequency_customers_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  return (
    <>
        ...
        <div className="p-6 border-b border-border-light">
          <div className="flex items-start justify-between">
            <div className="flex items-start space-x-4">
              <div className="p-3 bg-warning-50 rounded-soft">
                <UserMinusIcon className="h-6 w-6 text-warning-500" />
              </div>
              <div className="flex-1">
                <h3 className="text-base sm:text-lg font-semibold text-text-primary">
                  Low Frequency Customer Report
                </h3>
                <p className="text-sm text-text-secondary mt-1 leading-relaxed">
                  Identify customers with few orders for re-engagement campaigns
                </p>
              </div>
            </div>
          </div>
          ...
          {Object.keys(currentFilters).length > 0 && (
            <div className="mt-4 p-3 bg-warning-50 rounded-soft border border-warning-200">
              <p className="text-xs font-medium text-warning-700 mb-2">Active Filters:</p>
              <div className="flex flex-wrap gap-2">
                ...
                {currentFilters.minOrderCount !== undefined && currentFilters.maxOrderCount !== undefined && (
                  <span className="text-xs px-2 py-1 bg-surface-0 text-text-secondary rounded-soft border border-border-default">
                    Orders: {currentFilters.minOrderCount === currentFilters.maxOrderCount
                      ? `Exactly ${currentFilters.minOrderCount}`
                      : `${currentFilters.minOrderCount} to ${currentFilters.maxOrderCount}`}
                  </span>
                )}
              </div>
            </div>
      ...
      {/* Modals */}
      <LowFrequencyCustomerReportFilters
      ...
      <CSVPreviewTable
        ...
        reportTitle="Low Frequency Customer Report Preview"
